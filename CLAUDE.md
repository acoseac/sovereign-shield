...
- **Fail loudly when a transport moves** — the endpoint fingerprints in
  ...
  **The warning is retractable.** The poll keeps running for `CANARY_RETRACT_MS` (45 s) *after*
  ...
  the poll abandons ship the moment a **new** send intent arrives, since the next counter
  movement is that send's dispatch rather than a late inspect of this one; and a send may retract
  only a bar **it** raised (`showBanner`'s return value) — with one bar per page, an earlier
  genuinely-missed send's warning is still true, and a later send turning out fine must not
  erase it.
